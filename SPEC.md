...
**判「这不是译文」的主判据是 `usage.total_tokens == 0`**，不是词表。网关
的关键词扫描拦下请求时会连着 HTTP 200 回一段自己写的说明，但模型压根没
跑，token 消耗为 0 —— 这是客观信号。`looks_like_refusal` 退为兜底（拿不到
usage 的场合），且只认「结构上不可能是片名」的形态：完整话术短语、英文
元话语、日文原文却回英文散文、长度远超原文。

**绝不按单词判。** 线上为此误杀过三批正常译文：「违反」（校則違反是高频
题材）、「抱歉／对不起」（ごめんなさい是常见标题句式）、「无法满足」
（欲求不满题材）。片名可以是任何内容，任何词都可能合法出现，加词表就是
打不完的补丁。改判据前先想清楚：这个形态有没有可能是一个真实片名。
