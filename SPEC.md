...
**判「这不是译文」：不猜。** AI 路径按 JSON 契约工作 —— 提示词要求模型
返回 `{"zh": "译文"}`，要拒绝就走 `{"error": "..."}`。`parse_translation`
解析不出对象、或对象里没有 `zh`，就当失败降级到下一家。拒绝说明、思考
痕迹、原文回显、`**Simplified Chinese Translation:**` 这类标注、被网关砍
断的半截输出，全都是散文，长不成这个形状 —— 一条规则覆盖全部形态，且
zh 字段里的内容不管多离谱都照收，片名用词从此与判定无关。

契约之前还有一道客观信号：`usage.total_tokens == 0` 说明网关在关键词扫描
那层就把请求拦了，模型压根没跑。

存量清洗与 zh 字段校验走 `is_junk_title`：整句话术，**或含换行**。这是
唯一被允许的结构规则 —— 它看的是形状不是用词：原文标题是单行，译文里
出现换行只可能是模型把说明一并吐了出来（存量里实测有「译文 + 提示：…」
的混合体，一个话术都不命中）。这条规则不能进 `looks_like_refusal`，影评
模块拿它校验的是多行 JSON。

`looks_like_refusal` 已收成**纯整句话术匹配**（两张表：网关/模型的固定
拒绝话术、推理模型的思考痕迹与标注），不做任何结构或长度推断。它只剩
两处兜底：清洗存量（`purge_refused_translations`，带 20% 熔断）和 zh 字段
被硬塞了拒绝话术的极端情况。

**绝不按内容猜。** 为此误杀过四批正常译文：「违反」（校則違反是高频题材）、
「抱歉／对不起」（ごめんなさい是常见标题句式）、「无法满足」（欲求不满
题材）、VR/BOX/SP（片名自带的英文标记）。每次都是往词表或结构规则里加
条件，而片名可以是任何内容 —— 补丁永远打不完，而且清洗存量每轮把误判的
译文清掉再重翻，白烧 API。要动这块，先问：这条规则有没有可能命中一个
真实片名？答案不是「不可能」就别加。
